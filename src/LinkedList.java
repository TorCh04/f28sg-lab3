public class LinkedList {
	 ...
	 */
	public int size(){
		// Creates a temporary Node and count
		Node tempNode = headNode;
		int count = 0;
		// Repeats if the Node isn't empty
		while (tempNode != null)
		{
			// Increment Count
			count++;
			// Change tempNode to next Node along
	 ...
	 */
	public int total(){ 
		// Make a temporary Node and set the value to the head Node
		Node tempNode = headNode;
		int count = 0;
		// Repeats if temp Node isn't empty
		while (tempNode != null)
		{
			// Add count and the value of the current Node
			count = count + tempNode.getValue();
			// Change tempNode to the next Node
			tempNode = tempNode.getNextNode();
		}
		return count; 
	}
